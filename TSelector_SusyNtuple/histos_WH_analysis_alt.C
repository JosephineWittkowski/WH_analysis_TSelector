...
//   l1 <-> el
  ...
  for (uint im=0; im<MuonsAfterORWJet_vec.size(); im++){
    for(uint ij=0; ij<preJets.size(); ij++){
      double DeltaR_ljOR_EM = MuonsAfterORWJet_vec.at(im)->DeltaR(*preJets.at(ij));
